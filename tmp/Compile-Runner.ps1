...
param (
        ...
        [string]$ScriptPath="$PSScriptRoot\DownloadToolRunner.ps1",
        [Parameter(Mandatory = $false)]
        [string]$OutputPath="$PSScriptRoot\DownloadTool.exe",
        [Parameter(Mandatory = $false)]
        [switch]$Deploy,
        [Parameter(Mandatory = $false)]
        [switch]$TestBuild
...
try{
    ...
    if(Test-Path $OutputPath){
        ...
        if($Deploy){
            ...
            Write-Host "Copy-Item $OutputPath ==> $Env:ToolsRoot" -f DarkYellow
            Write-Host "Copy-Item $OutputPath ==> $Env:ToolsRoot\httpie.exe" -f DarkCyan
            Copy-Item "$OutputPath" "$Env:ToolsRoot" 
            Copy-Item "$OutputPath" "$Env:ToolsRoot\httpie.exe" 
            &"$expath" "$Env:ToolsRoot"
        }
    }else {
        throw "FAILURE"        
    }
